...
public class Game
{
    ...
    /// The result.
    /// </value>
    public GameResult Result { get; set; } = GameResult.Open;
    ...
    {
        return new Formatter(separator).Format(this);
    }

    public static Game FromPgn(string pgn) => GameHelpers.Create(pgn);

    public static Game FromPgnFile(string pgnFile) => FromPgn(System.IO.File.ReadAllText(pgnFile));
}